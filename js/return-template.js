...
customElements.define('return-template', class extends HTMLElement {
  get labelText() {
...
<div class="control-group">
  ...
  onClick() {
    ...
    fetch(new URL(`${this.systemPaths.baseFull}index.php?option=com_ajax&type=plugin&plugin=changetemplate&group=user&method=returntemplate&format=json&${this.token}=1`), {method: 'POST'})
    .then((response)=>{
...
// if this does not work, use jquery in the php file to change the title or button or use another butt.	 
	 if(func == 1)
		 ${this.buttonText('Delete text File')}
	 else
